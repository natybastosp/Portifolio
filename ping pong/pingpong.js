...
var colors = ["#8D6B94", "#B185A7", "#C3A29E", "#297045", "#B9F18C"];
  ...
  update: function () {
    if (!this.over) {
      ...
      if (this.ball.y >= this.canvas.height - this.ball.height)
        this.ball.moveY = directions.up;

      // Move player if they player.move value was updated by a keyboard event
      if (this.player.move === directions.up)
        this.player.y -= this.player.speed;
      else if (this.player.move === directions.down)
        this.player.y += this.player.speed;

      // On new serve (start of each turn) move the ball to the correct side
      // and randomize the directions to add some challenge.
      if (Pong._turnDelayIsOver.call(this) && this.turn) {
        this.ball.moveX =
          this.turn === this.player ? directions.left : directions.right;
        this.ball.moveY = [directions.up, directions.down][
      ...
      // Move ball in intended directions based on moveY and moveX values
      if (this.ball.moveY === directions.up)
        this.ball.y -= this.ball.speed / 1.5;
      else if (this.ball.moveY === directions.down)
        this.ball.y += this.ball.speed / 1.5;
      if (this.ball.moveX === directions.left) this.ball.x -= this.ball.speed;
      else if (this.ball.moveX === directions.right)
        this.ball.x += this.ball.speed;

      // Handle paddle (AI) up and down movement
      if (this.paddle.y > this.ball.y - this.paddle.height / 2) {
        if (this.ball.moveX === directions.right)
  ...
  listen: function () {
    document.addEventListener("keydown", function (key) {
      ...
      if (key.keyCode === 38 || key.keyCode === 87)
        Pong.player.move = directions.up;

      // Handle down arrow and s key events
      if (key.keyCode === 40 || key.keyCode === 83)
        Pong.player.move = directions.down;
    });

    // Stop the player from moving when there are no keys being pressed.
    document.addEventListener("keyup", function (key) {
      Pong.player.move = directions.idle;
    });
  },
